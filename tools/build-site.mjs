...
const url = slug => `/articles/${slug}.html`;
const guideUrl = url('audible-beginners-guide');
const affiliatePanel = `<section class="affiliate-panel" aria-label="Audibleのご案内"><span class="eyebrow">PR · AUDIBLE</span><h2>まずは無料体験で、聴いてみる。</h2><p>Audibleの無料体験で、気になる作品を。<br>自分に合う聴き方を試してみませんか。</p><a class="affiliate-button" href="${esc(affiliate.link)}" target="_blank" rel="sponsored nofollow noopener noreferrer">Audibleの無料体験を試す <span aria-hidden="true">↗</span></a><small>無料体験は対象の方に適用されます。体験期間・終了後の料金は申込画面をご確認ください。体験終了後は自動更新されます。</small></section>`;
...
function shell(title, description, main) {
  return `<!doctype html><html lang="ja"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width, initial-scale=1"><title>${esc(title)}</title><meta name="description" content="${esc(description)}">${preview ? '<meta name="robots" content="noindex,nofollow">' : ''}<meta name="theme-color" content="#f4f3ee"><link rel="preconnect" href="https://fonts.googleapis.com"><link rel="preconnect" href="https://fonts.gstatic.com" crossorigin><link href="https://fonts.googleapis.com/css2?family=Noto+Sans+JP:wght@400;500&family=Noto+Serif+JP:wght@400;500;600&display=swap" rel="stylesheet"><link rel="stylesheet" href="/css/journal.css"></head><body><a class="skip-link" href="#main">本文へ移動</a>${preview ? '<div class="preview-note">リニューアル確認用</div>' : ''}<header class="site-header"><div class="header-inner">${brand}<nav aria-label="メインメニュー"><a href="/#journal">記事一覧</a><a href="${guideUrl}">はじめてのAudible</a><a href="/#about">耳読ライフについて</a></nav></div></header>${affiliateDisclosure}${main}<footer class="site-footer"><div>${brand}<p>聴く読書のある、いつもの暮らし。</p></div><small>© 2026 耳読ライフ</small></footer>${affiliatePixel}</body></html>`;
}
const card = a => `<a class="story-card" href="${url(a.slug)}">${photo(a.image, a.imageAlt, a.imageNote)}<div class="story-copy"><span class="category">${esc(a.category)}</span><h3>${esc(a.title)}</h3><p>${esc(a.description)}</p><span class="story-meta"><time datetime="${a.date}">${a.date.replaceAll('-', '.')}</time><span aria-hidden="true">読む →</span></span></div></a>`;
...
for (const a of articles) {
  ...
  const html = shell(`${a.title}｜耳読ライフ`, a.description, `<main id="main" class="article-main"><a class="back-link" href="/#journal">← 記事一覧へ戻る</a><div class="article-layout"><article class="article"><header class="article-header"><span class="category">${esc(a.category)}</span><h1>${esc(a.title)}</h1><p class="article-meta"><time datetime="${a.date}">${a.date.replaceAll('-', '.')}</time>${site.author ? ` · ${esc(site.author)}` : ''}${a.checkedDate ? `<span>情報確認日：${a.checkedDate.replaceAll('-', '.')}</span>` : ''}</p>${photo(a.image, a.imageAlt, a.imageNote)}</header><details class="toc"><summary>この記事の目次</summary><ol>${a.toc.join('')}</ol></details><div class="article-body">${a.body}</div>${affiliatePanel}<section class="next-reading"><span class="eyebrow">NEXT READING</span><h2>次に読む</h2>${related.map(x => `<a href="${url(x.slug)}">${esc(x.title)} <span aria-hidden="true">→</span></a>`).join('')}</section></article><aside class="sidebar"><section>${about}</section><section><span class="eyebrow">READ ALSO</span>${related.map(x => `<a class="sidebar-story" href="${url(x.slug)}"><span class="category">${esc(x.category)}</span><h2>${esc(x.title)}</h2><span aria-hidden="true">読む →</span></a>`).join('')}</section></aside></div></main>`);
